...
OODA is WF’s lightweight job scheduler and one-shot runner.
...
wf ooda run-job --job my-job-id --config .waif/ooda-scheduler.yaml --log history/ooda_snapshot.jsonl --json
```
...
Running `wf ooda` with **no** subcommand defaults to running the scheduler.
...
```bash
wf --json ooda
wf --json ooda scheduler --config .waif/ooda-scheduler.yaml --interval 60
wf --json ooda run-job --job my-job-id
```
...
- For tests and CI, prefer `wf ooda run-job --job <id>` (deterministic, exits).
...
- [ ] Doc explains that `wf ooda` defaults to `scheduler`.
- [ ] Doc includes examples for `wf ooda`, `wf ooda scheduler ...`, and `wf ooda run-job ...`.
...
- Confirm the scheduler loop is running (`wf ooda schedule status`).
