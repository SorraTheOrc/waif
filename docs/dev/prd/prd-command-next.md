...
* One-liner
  * CLI tool `wf` exposing subcommand `next` that returns the single best issue to work on now, printing a compact human output and supporting `--json` machine output.
...
* Goals
  ...
  * Provide `--json` output matching `bd`'s JSON issue format, with computed scoring metadata included under a top-level `wf` metadata object for automation.
...
* Secondary users (optional)
  * AI orchestrator / automation tooling — consumes `wf next --json` for programmatic decision-making.

* Key user journeys
  * PM runs: `wf next` → sees a one-row recommendation table plus a details section (from `bd show` when available).
  * PM runs: `wf next --json` → receives bd-format JSON augmented with a top-level `wf` object containing computed score and ranking metadata.
  * Automation: scheduled agent runs `wf next --json` to seed downstream orchestration workflows.

## Requirements

* Functional requirements (MVP)

1. `wf next` lists eligible candidates (open and unblocked) and returns the top-ranked issue. When `--assignee <name>` is provided, eligible candidates are limited to issues whose `assignee` matches the provided name (after trim), and the existing selection/ranking logic is applied to that filtered set.
2. `wf next` prints human-readable output as:
...
3. `wf next --json` prints a JSON object with full issue details plus computed score and ranking metadata.
...
6. On success, `wf next` copies the recommended issue id to the OS clipboard (best-effort).
...
* Integrations
  ...
  * CLI implementation must follow the repo's CLI conventions and be runnable as `wf next` subcommand.
...
* Quality gates / definition of done
  ...
  * PR reviewed and merged; `wf` documented in README/CLI docs.
...
This PRD is updated to cover the intake for adding an optional search string that allows the Producer to bias `wf next` recommendations. The Source issue for this intake is: wf-c2n.
...
### Users
Primary: Producer (user running `wf next`). No secondary stakeholders identified.

### Acceptance criteria
1. `wf next` accepts an optional positional search string: `wf next "<search>"`.
...
6. CLI UX: search string is a positional argument (e.g., `wf next "ui bug"`). Optional `--assignee <name>` applies a hard filter first; the positional search then re-ranks within the filtered set.
7. `wf next --json` must preserve its existing JSON contract and must NOT include additional match metadata in v1. JSON error path for `--assignee` no-match should return `{ "error": { "message", "code" } }` aligned with other commands.
